"""Standalone SQLite-based Document Database - No external dependencies."""
import json
import logging
import sqlite3
...
class SQLiteDocumentDatabase:
    ...
    def __init__(self, db_path: str = "documents.db", vector_store=None, config=None):
        """Initialize the SQLite document database.

        Args:
            db_path: Path to the SQLite database file
            vector_store: Optional vector store instance (for compatibility)
    ...
    def search_semantic(self, query_embedding: List[float], top_k: int = 10,
        ...
        if self.config:
            if threshold is None:
                threshold = getattr(self.config, 'semantic_search_min_threshold', -1.0)
            if max_candidates is None:
                max_candidates = getattr(self.config, 'semantic_search_max_candidates', 50)
            debug_enabled = getattr(self.config, 'semantic_search_debug', False) or logger.isEnabledFor(logging.DEBUG)
        else:
            if threshold is None:
                threshold = -1.0
            if max_candidates is None:
