...
python src/backend/api/app.py  # APIs on port 8081
...
# Flask API runs separately: python src/backend/api/app.py
```
...
```bash
python src/backend/scripts/simple_migrate.py
```
...
**Command Line:**
```bash
sqlite3 data/databases/documents.db
.tables
.schema documents
...
5. Configure the agent by editing `src/backend/config/config.yaml`:
```yaml
source_paths:
  - "data/input"                # Where to read files from
database:
  path: "data/databases/documents.db"  # High-performance SQLite database file
  json_export_path: "data/exports/classifications.json"  # JSON export file
ollama:
...
python src/backend/api/app.py --port 8080 --host 127.0.0.1

# Use custom config and disable debug mode
python src/backend/api/app.py --config src/backend/config/custom_config.yaml --no-debug

# Command line options override config file settings
python src/backend/api/app.py --port 3000 --debug
```

The web app will be available at the configured host and port (default: `http://localhost:8081`) with the following features:
...
- **source_paths**: Directories to monitor for files (list)
- **database**: Database settings
  - **path**: Path to SQLite database file (default: `data/databases/documents.db`)
  - **json_export_path**: Path for JSON export file (default: `data/exports/classifications.json`)
...
Open your browser to the configured URL (default: `http://localhost:8081`) to access the web interface with semantic search, document browsing, and statistics.

## Troubleshooting

### Ollama Connection Error
- Ensure Ollama is running: `ollama serve`
- Check the endpoint in `src/backend/config/config.yaml` matches your Ollama setup
- Verify the models are installed: `ollama list`

### Embedding Model Not Found
- Pull the embedding model: `ollama pull qwen3-embedding:8b`
- Check the model name in `src/backend/config/config.yaml` matches an installed model
...
### Database Errors
- Check file permissions on the SQLite database file (`data/databases/documents.db`)
- Ensure the database directory is writable
- Check logs for specific error messages
- Use `sqlite3 data/databases/documents.db "PRAGMA integrity_check;"` to verify database integrity
...
- For reasoning models (like deepseek-r1), increase `num_predict` in `src/backend/config/config.yaml`
- If `done_reason: 'length'` appears, the model hit the token limit - increase `num_predict`
- Try customizing the `prompt_template` in `src/backend/config/config.yaml`
...
- Check the configured host and port in `src/backend/config/config.yaml` under `webapp` section
- Run with debug mode: `python src/backend/api/app.py --debug` (includes error details)
